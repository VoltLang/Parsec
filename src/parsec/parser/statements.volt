...
module parsec.parser.statements;
...
import parsec.ir.copy : copyType;

import parsec.errors;
import parsec.exceptions;
import parsec.lex.writer;
import parsec.lex.token : TokenType, Token;
...
ParseStatus parseBlockStatement(ParserStream ps, out ir.BlockStatement bs)
{
	...
	if (matchIf(ps, TokenType.OpenBrace)) {
		while (ps != TokenType.CloseBrace) {
			auto succeeded = parseStatement(ps, out sink.push);
			if (!succeeded) {
...
ParseStatus parseForStatement(ParserStream ps, out ir.ForStatement f)
{
	...
	if (ps != TokenType.Semicolon) {
		...
		auto sink = new NodeSink();
		auto succeeded = parseVariableOrExpression(ps, out sink.push);
		if (!succeeded) {
			return parseFailed(ps, f);
		}
